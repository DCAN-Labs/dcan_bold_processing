...
# Potential issue: the GSR version is slightly different from the saved file in the original folder (ABCD) potentially because of the differences in interpolation and padding?
...
def main():
    ...
    parser.add_argument('--subject-id',type=str, help='The folder name for subject, e.g. sub-xxxxxx',required=True)
    parser.add_argument('--task-id',type=str, help='The folder name for run e.g. task-rest01',required=True)
    parser.add_argument('--data-path',type=str, help='DCAN ABCD-HCP-pipeline output folder',required=True)
    ...
    if not args.json_path: # Use the default for ABCD on WashU NIL drive
        args.json_path = os.path.join(data_path,subject_id, 'ses-baselineYear1Arm1/files/MNINonLinear/Results/',task_id,'DCANBOLDProc_v4.0.0/DCANBOLDProc_v4.0.0_mat_config.json')
    ...
    if json_input['GSR']:
        ...
        json_input['result_dir'] = os.path.join(results_path,subject_id,task_id,'DCANBOLDProc_v4.0.0','nogsr')
    json_input['path_cii'] = str.replace(json_input['path_cii'],'/output/',data_path)
    json_input['file_mov_reg'] = str.replace(json_input['file_mov_reg'],'/output/',data_path) # This should be the filtered movement parameters (columns 3-6 in degrees) if you use the filtered version
    json_input['file_vent'] = str.replace(json_input['file_vent'],'/output/',data_path)
    ...
    logging.basicConfig(filename=os.path.join(json_input['result_dir'],'output.log'),level=logging.INFO,format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    if json_input['GSR']:
        ...
        plt.savefig(os.path.join(json_input['result_dir'],'globalsignal_trace.png'), format='png', dpi = 300)
    ...
    assert os.path.isfile(json_input['file_mov_reg']), 'movement regressor file not found' 
    filtered_mov_regressors = np.loadtxt(json_input['file_mov_reg'])
    FR = make_friston_regressors(filtered_mov_regressors,json_input['brain_radius_mm']) # 24-param Friston Regressors

    # Calculate FD  (Line 73 in dcan_bold_processing.m)
    FD,meanFD = calc_FD(filtered_mov_regressors,FD_type=json_input['FD_type'])
    FD_file_name = os.path.join(json_input['result_dir'],json_input['FNL_preproc_CIFTI_basename'] + '_FD.txt')
    ...
    if json_input['GSR']:
        R = np.concatenate((Glob,dGlob,FR),axis = 1) # with GSR # N.B. this uses 30 parameter, for 36 parameter it would also involve the square of Glob and dGlob
    else:
        R = FR # without GSR
    ...
    plt.plot(data_postreg[:,0],linewidth=2,label='example original signal')
    plt.plot(data_interpolated[:,0],linewidth=2,label='example interpolated signal')
    plt.xlabel('TR')
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig(os.path.join(json_input['result_dir'],'interpolation_trace.png'), format='png', dpi = 300)

    # Bandpass filter with manual zero padding
    fs = 1 / TR # Sampling frequency
    fNy = fs / 2 # Nyquist frequency
    b_filt,a_filt = scipy.signal.butter(json_input['bp_order']/2,np.array([json_input['lp_Hz'],json_input['hp_Hz']])/fNy,'bandpass')
    ...
    plt.figure(figsize=(8,4))
    plt.plot(DVAR_pre_reg,linewidth=2,label='DVARS pre regression',color='b')
    plt.plot(DVAR_post_reg,linewidth=2,label='DVARS post regression',color='r')
    plt.plot(DVAR_post_filter,linewidth=2,label='DVARS post filtered',color='g')
    plt.xlabel('TR')
    plt.legend()
    plt.savefig(os.path.join(json_input['result_dir'],'DVARS_trace.png'), format='png', dpi = 300)
    ...
    output_img.to_filename(os.path.join(json_input['result_dir'],json_input['FNL_preproc_CIFTI_basename'] + '.dtseries.nii'))
