function subject_FD_parse_v3(FD_movement_files,skip_seconds,epi_TR,brain_radius_in_mm,varargin)
...
FD_range = 0:0.01:0.5; 
FD_data = {};
run_count = size(FD_movement_files,2); % each movement file is one run
...
for i = 1:length(FD_range) % for each FD threshold
    ...
    for j = 1:run_count % for each run/series
        ...
        switch movement_file(end-3:end)
            ...
            case '.dat' % Calculate FD values from 4dfp DAT files
    ...
    if ~isempty(varargin)
        mask = varargin{1};
        if length(mask) == length(FD_data{i}.frame_removal)
            FD_data{i}.frame_removal = (FD_data{i}.frame_removal + mask) > 0;
        end
    end
    ...
    FD_data{i}.remaining_frame_mean_FD = mean(f(FD_data{i}.frame_removal==0)); % calculate the mean FD of remaining frames
...
if isempty(varargin)
    ...
    save([this_path filesep 'FD_masked.mat'],'FD_data');
end
