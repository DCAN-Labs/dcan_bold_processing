function subject_power_2014_FD_only_parse_v3_gaby(movement_files,skip_seconds,epi_TR,expected_contiguous_frame_count,varargin)
...
% values appropriately in the 5 1x51x3 cell array of structs
%
...
for i = 1:length(FD_range)
    ...
    if ~isempty(varargin)
        ...
        load(mask);
        mask = SRP;
        if length(mask) == length(motion_data{i}.frame_removal)
            motion_data{i}.frame_removal = (motion_data{i}.frame_removal + mask) > 0;
        end
    end
...
% Save output motion_data cell array of structs to an power_2014_FD_only.mat file in the present working directory
this_path=pwd;
if isempty(varargin)
    save([this_path filesep 'power_2014_FD_only.mat'],'motion_data');
else
    save([this_path filesep 'power_2014_FD_only_masked.mat'],'motion_data');
end
