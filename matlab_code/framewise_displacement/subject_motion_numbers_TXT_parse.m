...
function subject_motion_numbers_TXT_parse(movement_files)
...
for i = 1:run_count
    ...
    switch movement_file(end-3:end)
        case '.txt'
            ...
            motion_cell = textscan(fid, '%*s %*s %*s %f %f %f %f %f %f %f %f','HeaderLines',1);% Read FD values from motion numbers files
            fclose(fid);
    ...
    motion_numbers.DVAR = cat(1,motion_numbers.DVAR,DVAR);
    
end

%%
save('motion_numbers.mat','motion_numbers');
