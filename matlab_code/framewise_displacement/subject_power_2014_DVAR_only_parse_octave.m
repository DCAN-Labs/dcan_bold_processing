function subject_power_2014_DVAR_only_parse_octave(movement_files,skip_seconds,epi_TR,expected_contiguous_frame_count,output_dir)
    ...
    for run = 1:run_count
        ...
        switch movement_file(end-3:end)
            case '.txt' % Read FD values from motion numbers files
                ...
                motion_cell = textscan(fid, '%*s %*s %*s %*s %*s %*s %*s %*s %*s %f %*s %*s %*s','HeaderLines',1);% Read FD values from motion numbers files
                fclose(fid);
...
save([output_dir filesep 'power_2014_FD_only.mat'],'motion_data','-v7');
